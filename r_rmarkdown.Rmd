...
The hotkeys for inserting code chunk are <span> Ctrl + Alt + I </span>, while the hotkeys for knitting are Ctrl + Shift + K.\
Always give the chunk a name <span> {r chunk_name} </span>, so when we have bugs, we can look back and find that chunk immediately.
...
Inserting brief code snippets inline is often helpful; we can use them for a brief summary of the sample size or statistics in text by inserting code inline using <span> `r ` </span>, often in conjunction with the format() function to clean up the output.
